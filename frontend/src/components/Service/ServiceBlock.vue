        ...
        <div class="col-12">
          <h4 class="mt-2">
            <router-link
              ...
              class="d-inline-block text-truncate font-4"
        ...
        <div class="col-md-10 col-6">
          ...
          >
            <button
              ...
              @click.prevent="openMenu('timeframe')"
            >
              {{ timeframepick.text }}
            </button>
            <div
              class="service-tm-menu"
              :class="{'d-none': !dropDownMenu}"
            >
              <a
                v-for="(timeframe, i) in timeframes"
                class="dropdown-item"
                href="#"
                :class="{'active': timeframepick === timeframe}"
                @click.prevent="changeTimeframe(timeframe)"
              >
                {{ timeframe.text }}
              </a>
            </div>
          ...
          <div
            class="dropup"
            :class="{show: intervalMenu}"
          >
            <button
              style="font-size: 10pt;"
              type="button"
              class="col-4 float-left btn btn-sm float-right btn-block text-white dropdown-toggle service_scale pr-2"
              @click.prevent="openMenu('interval')"
            >
              {{ intervalpick.text }}
            </button>
            <div
              class="service-tm-menu"
              :class="{'d-none': !intervalMenu}"
            >
              <a
                v-for="(interval, i) in intervals"
                class="dropdown-item"
                href="#"
                :class="{'active': intervalpick === interval, 'disabled': disabled_interval(interval)}"
                @click.prevent="changeInterval(interval)"
              >
                {{ interval.text }}
              </a>
            </div>
...
export default {
    ...
    methods: {
        disabled_interval (interval) {
            const min = this.timeframepick.set - interval.set - 1;
